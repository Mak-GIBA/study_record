...
# ソフトマックス関数
def softmax(x):
    if x.ndim == 2: #ミニバッチとしてデータを処理する際は、行列の形が違うため条件分岐
        x = x.T
        x = x - np.max(x, axis=0) # 列方向の最大値で正規化
        # ①出力値、②配列xにexpをとる、②全てのクラスの総和を取る
...
def cross_entropy_error(d, y):
    ...
    # logの中身は(y[np.arange(batch_size), d] + 1e-7)の部分、dはニューラルネットの出力
